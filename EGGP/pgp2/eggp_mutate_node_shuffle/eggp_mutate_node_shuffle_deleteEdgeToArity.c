...
bool eggp_mutate_node_shuffle_b4 = true;

static bool evaluateCondition(void)
{
   return (eggp_mutate_node_shuffle_b4);
}

static void evaluatePredicateeggp_mutate_node_shuffle_4(Morphism *morphism)
{
   ...
   if(var_4 > var_5) eggp_mutate_node_shuffle_b4 = true;
   else eggp_mutate_node_shuffle_b4 = false;
}
...
bool matcheggp_mutate_node_shuffle_deleteEdgeToArity(Morphism *morphism)
{
   if(4 > eggp_mutate_node_shuffle_host->number_of_nodes || 1 > eggp_mutate_node_shuffle_host->number_of_edges) return false;
...
{
   RootNodes *nodes;
   for(nodes = getRootNodeList(eggp_mutate_node_shuffle_host); nodes != NULL; nodes = nodes->next)
   {
      Node *host_node = getNode(eggp_mutate_node_shuffle_host, nodes->index);
      ...
      do
      {
         ...
         if(result >= 0)
         {
            ...
            evaluatePredicateeggp_mutate_node_shuffle_4(morphism);
            if(!evaluateCondition())
            {
               /* Reset the boolean variables in the predicates of this variable. */
               eggp_mutate_node_shuffle_b4 = true;
               break;
            }
         }
         else break;
...
static bool match_e0(Morphism *morphism)
{
   ...
   Node *host_node = getNode(eggp_mutate_node_shuffle_host, start_index);

   int counter;
   for(counter = 0; counter < host_node->out_edges.size + 2; counter++)
   {
      Edge *host_edge = getNthOutEdge(eggp_mutate_node_shuffle_host, host_node, counter);
      ...
      {
         Node *end_node = getNode(eggp_mutate_node_shuffle_host, host_edge->target);
...
static bool match_n1(Morphism *morphism, Edge *host_edge)
{
   Node *host_node = getTarget(eggp_mutate_node_shuffle_host, host_edge);
...
void applyeggp_mutate_node_shuffle_deleteEdgeToArity(Morphism *morphism, bool record_changes)
{
   int var_4 = getIntegerValue(morphism, 4);
   int host_edge_index = lookupEdge(morphism, 0);
   if(record_changes)
   {
      Edge *edge = getEdge(eggp_mutate_node_shuffle_host, host_edge_index);
   ...
   removeEdge(eggp_mutate_node_shuffle_host, host_edge_index);

   int host_node_index = lookupNode(morphism, 2);
   HostLabel label_n2 = getNodeLabel(eggp_mutate_node_shuffle_host, host_node_index);
   HostLabel label;
   ...
   else
   {
      ...
      relabelNode(eggp_mutate_node_shuffle_host, host_node_index, label);
   }
   /* Reset the morphism. */
   initialiseMorphism(morphism, eggp_mutate_node_shuffle_host);
}
...
bool fillpoteggp_mutate_node_shuffle_deleteEdgeToArity(MorphismPot *pot, Morphism *morphism)
{
   if(4 > eggp_mutate_node_shuffle_host->number_of_nodes || 1 > eggp_mutate_node_shuffle_host->number_of_edges) return false;
...
static bool fillpot_n1(MorphismPot *pot, Morphism *morphism, Edge *host_edge)
{
   ...
   if(match)
   {
      ...
      host_node->matched = true;
      /* All items matched! */
         putMorphism(pot, morphism, "eggp_mutate_node_shuffle_deleteEdgeToArity", eggp_mutate_node_shuffle_host);
