...
bool eggp_mutate_node_shuffle_b2 = true;

static bool evaluateCondition(void)
{
   return (eggp_mutate_node_shuffle_b2);
}

static void evaluatePredicateeggp_mutate_node_shuffle_2(Morphism *morphism)
{
   ...
   if(!equalHostLists(array0, array1, list_length0, list_length1)) eggp_mutate_node_shuffle_b2 = true;
   else eggp_mutate_node_shuffle_b2 = false;
}

static bool match_n1(Morphism *morphism);
static bool match_n0(Morphism *morphism);

bool matcheggp_mutate_node_shuffle_mutateNode(Morphism *morphism)
{
   ...
   for(nodes = getRootNodeList(eggp_mutate_node_shuffle_host); nodes != NULL; nodes = nodes->next)
   {
      ...
      do
      {
         ...
         if(result >= 0)
         {
            ...
            evaluatePredicateeggp_mutate_node_shuffle_2(morphism);
            if(!evaluateCondition())
            {
               /* Reset the boolean variables in the predicates of this variable. */
               eggp_mutate_node_shuffle_b2 = true;
               break;
            }
         }
         else break;
...
void applyeggp_mutate_node_shuffle_mutateNode(Morphism *morphism, bool record_changes)
{
   Assignment var_0 = getAssignment(morphism, 0);
   string var_2 = getStringValue(morphism, 2);
   int var_3 = getIntegerValue(morphism, 3);
   int node_index = lookupNode(morphism, 0);
   int outdegree0 = getOutdegree(eggp_mutate_node_shuffle_host, node_index);
   int host_node_index = lookupNode(morphism, 0);
   HostLabel label_n0 = getNodeLabel(eggp_mutate_node_shuffle_host, host_node_index);
   HostLabel label;
   int list_var_length0 = 0;
   ...
   int node_array_size2 = eggp_mutate_node_shuffle_host->nodes.size;
   int list_var_length1 = 0;
   ...
   host_node_index = addNode(eggp_mutate_node_shuffle_host, 1, label);
   /* If the node array size has not increased after the node addition, then
      the node was added to a hole in the array. */
   if(record_changes)
      pushAddedNode(host_node_index, node_array_size2 == eggp_mutate_node_shuffle_host->nodes.size);
   int node_array_size3 = eggp_mutate_node_shuffle_host->nodes.size;
   ...
   /* If the node array size has not increased after the node addition, then
      the node was added to a hole in the array. */
   if(record_changes)
      pushAddedNode(host_node_index, node_array_size3 == eggp_mutate_node_shuffle_host->nodes.size);
   /* Reset the morphism. */
   initialiseMorphism(morphism, eggp_mutate_node_shuffle_host);
}
...
bool fillpoteggp_mutate_node_shuffle_mutateNode(MorphismPot *pot, Morphism *morphism)
{
   if(2 > eggp_mutate_node_shuffle_host->number_of_nodes || 0 > eggp_mutate_node_shuffle_host->number_of_edges) return false;
...
static bool fillpot_n0(MorphismPot *pot, Morphism *morphism)
{
   ...
   for(host_index = 0; host_index < eggp_mutate_node_shuffle_host->nodes.size; host_index++)
   {
      ...
      if(match)
      {
         ...
         host_node->matched = true;
         /* All items matched! */
            putMorphism(pot, morphism, "eggp_mutate_node_shuffle_mutateNode", eggp_mutate_node_shuffle_host);
