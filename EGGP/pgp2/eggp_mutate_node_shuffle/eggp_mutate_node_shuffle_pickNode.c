...
bool eggp_mutate_node_shuffle_b0 = true;
bool eggp_mutate_node_shuffle_b1 = true;

static bool evaluateCondition(void)
{
   return (eggp_mutate_node_shuffle_b0 && eggp_mutate_node_shuffle_b1);
}

static void evaluatePredicateeggp_mutate_node_shuffle_0(Morphism *morphism)
{
   ...
   if(!equalHostLists(array0, array1, list_length0, list_length1)) eggp_mutate_node_shuffle_b0 = true;
   else eggp_mutate_node_shuffle_b0 = false;
}

static void evaluatePredicateeggp_mutate_node_shuffle_1(Morphism *morphism)
{
   Assignment assignment_1 = getAssignment(morphism, 1);
   /* If the variable is not yet assigned, return. */
   if(assignment_1.type == 'n') return;
   ...
   if(!equalHostLists(array0, array1, list_length0, list_length1)) eggp_mutate_node_shuffle_b1 = true;
   else eggp_mutate_node_shuffle_b1 = false;
}

static bool match_n0(Morphism *morphism);

bool matcheggp_mutate_node_shuffle_pickNode(Morphism *morphism)
{
   if(1 > eggp_mutate_node_shuffle_host->number_of_nodes || 0 > eggp_mutate_node_shuffle_host->number_of_edges) return false;
   ...
   for(host_index = 0; host_index < eggp_mutate_node_shuffle_host->nodes.size; host_index++)
   {
      ...
      do
      {
         ...
         if(result >= 0)
         {
            ...
            evaluatePredicateeggp_mutate_node_shuffle_0(morphism);
            evaluatePredicateeggp_mutate_node_shuffle_1(morphism);
            if(!evaluateCondition())
            {
               /* Reset the boolean variables in the predicates of this variable. */
               eggp_mutate_node_shuffle_b0 = true;
               eggp_mutate_node_shuffle_b1 = true;
               break;
            }
         }
         else break;
...
void applyeggp_mutate_node_shuffle_pickNode(Morphism *morphism, bool record_changes)
{
   ...
   changeNodeMark(eggp_mutate_node_shuffle_host, host_node_index, 3);
...
bool fillpoteggp_mutate_node_shuffle_pickNode(MorphismPot *pot, Morphism *morphism)
{
   if(1 > eggp_mutate_node_shuffle_host->number_of_nodes || 0 > eggp_mutate_node_shuffle_host->number_of_edges) return false;
...
static bool fillpot_n0(MorphismPot *pot, Morphism *morphism)
{
   ...
   for(host_index = 0; host_index < eggp_mutate_node_shuffle_host->nodes.size; host_index++)
   {
      ...
      if(match)
      {
         ...
         host_node->matched = true;
         /* All items matched! */
            putMorphism(pot, morphism, "eggp_mutate_node_shuffle_pickNode", eggp_mutate_node_shuffle_host);
