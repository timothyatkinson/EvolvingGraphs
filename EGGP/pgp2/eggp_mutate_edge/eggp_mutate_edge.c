...
int eggp_mutate_edge_execute(Graph* host_graph)
{
   ...
   M_eggp_mutate_edge_mutateEdge = makeMorphism(3, 1, 4);
   ...
   do
   {
      ...
      /* Rule Call */
      emptyPot(eggp_mutate_edge_pot);
      fillpoteggp_mutate_edge_pickNewTarget(eggp_mutate_edge_pot, M_eggp_mutate_edge_pickNewTarget);
      if(potSize(eggp_mutate_edge_pot) > 0){
         MorphismHolder *holder = drawFromPot(eggp_mutate_edge_pot);
         duplicateMorphism(holder->morphism, M_eggp_mutate_edge_pickNewTarget, eggp_mutate_edge_host);
         freeMorphism(holder->morphism);
         free(holder);
         applyeggp_mutate_edge_pickNewTarget(M_eggp_mutate_edge_pickNewTarget, true);
         eggp_mutate_edge_success = true;
      }
      else
      {
      ...
      emptyPot(eggp_mutate_edge_pot);
      /* Rule Call */
      if(matcheggp_mutate_edge_mutateEdge(M_eggp_mutate_edge_mutateEdge))
      {
