...
bool eggp_dc_init_b0 = true;

static bool evaluateCondition(void)
{
   return (eggp_dc_init_b0);
}

static void evaluatePredicateeggp_dc_init_0(Morphism *morphism)
{
   ...
   if(var_1 > 0) eggp_dc_init_b0 = true;
   else eggp_dc_init_b0 = false;
}

static bool match_n0(Morphism *morphism);
static bool match_n1(Morphism *morphism);

bool matcheggp_dc_init_addNode(Morphism *morphism)
{
...
void applyeggp_dc_init_addNode(Morphism *morphism, bool record_changes)
{
   ...
   HostLabel label_n1 = getNodeLabel(eggp_dc_init_host, host_node_index);
   ...
   int node_array_size2 = eggp_dc_init_host->nodes.size;
   ...
   host_node_index = addNode(eggp_dc_init_host, 0, label);
   /* If the node array size has not increased after the node addition, then
      the node was added to a hole in the array. */
   if(record_changes)
      pushAddedNode(host_node_index, node_array_size2 == eggp_dc_init_host->nodes.size);
   /* Reset the morphism. */
   initialiseMorphism(morphism, eggp_dc_init_host);
}
...
bool fillpoteggp_dc_init_addNode(MorphismPot *pot, Morphism *morphism)
{
   if(2 > eggp_dc_init_host->number_of_nodes || 0 > eggp_dc_init_host->number_of_edges) return false;
...
static bool fillpot_n1(MorphismPot *pot, Morphism *morphism)
{
   ...
   for(nodes = getRootNodeList(eggp_dc_init_host); nodes != NULL; nodes = nodes->next)
   {
      ...
      if(match)
      {
         ...
         host_node->matched = true;
         /* All items matched! */
            putMorphism(pot, morphism, "eggp_dc_init_addNode", eggp_dc_init_host);
