...
bool matcheggp_dc_init_deleteNodeCounter(Morphism *morphism)
{
...
void applyeggp_dc_init_deleteNodeCounter(Morphism *morphism, bool record_changes)
{
   int count;
   for(count = 0; count < morphism->edges; count++)
   {
      if(record_changes)
      {
         Edge *edge = getEdge(eggp_dc_init_host, morphism->edge_map[count].host_index);
         ...
         pushRemovedEdge(edge->label, edge->source, edge->target, edge->index,
                         edge->index < eggp_dc_init_host->edges.size - 1);
      }
      removeEdge(eggp_dc_init_host, morphism->edge_map[count].host_index);
   }
   for(count = 0; count < morphism->nodes; count++)
   {
      if(record_changes)
      {
         Node *node = getNode(eggp_dc_init_host, morphism->node_map[count].host_index);
         /* A hole is created if the node is not at the right-most index of the array. */
         pushRemovedNode(node->root, node->label, node->index,
                         node->index < eggp_dc_init_host->nodes.size - 1);
      }
      removeNode(eggp_dc_init_host, morphism->node_map[count].host_index);
   }
...
bool fillpoteggp_dc_init_deleteNodeCounter(MorphismPot *pot, Morphism *morphism)
{
   if(1 > eggp_dc_init_host->number_of_nodes || 0 > eggp_dc_init_host->number_of_edges) return false;
...
static bool fillpot_n0(MorphismPot *pot, Morphism *morphism)
{
   ...
   for(nodes = getRootNodeList(eggp_dc_init_host); nodes != NULL; nodes = nodes->next)
   {
      ...
      if(match)
      {
         ...
         host_node->matched = true;
         /* All items matched! */
            putMorphism(pot, morphism, "eggp_dc_init_deleteNodeCounter", eggp_dc_init_host);
