...
bool eggp_dc_init_b1 = true;
bool eggp_dc_init_b2 = true;
bool eggp_dc_init_b3 = true;

static bool evaluateCondition(void)
{
   return ((eggp_dc_init_b1 && eggp_dc_init_b2) && eggp_dc_init_b3);
}

static void evaluatePredicateeggp_dc_init_1(Morphism *morphism)
{
   ...
   if(!equalHostLists(array0, array1, list_length0, list_length1)) eggp_dc_init_b1 = true;
   else eggp_dc_init_b1 = false;
}

static void evaluatePredicateeggp_dc_init_2(Morphism *morphism)
{
   ...
   if(getOutdegree(eggp_dc_init_host, n0) < var_4) eggp_dc_init_b2 = true;
   else eggp_dc_init_b2 = false;
}

static void evaluatePredicateeggp_dc_init_3(Morphism *morphism)
{
   ...
   if(var_5 < var_6) eggp_dc_init_b3 = true;
   else eggp_dc_init_b3 = false;
}
...
bool matcheggp_dc_init_addEdgeToArity(Morphism *morphism)
{
   if(4 > eggp_dc_init_host->number_of_nodes || 0 > eggp_dc_init_host->number_of_edges) return false;
...
{
   RootNodes *nodes;
   for(nodes = getRootNodeList(eggp_dc_init_host); nodes != NULL; nodes = nodes->next)
   {
      Node *host_node = getNode(eggp_dc_init_host, nodes->index);
   ...
   for(nodes = getRootNodeList(eggp_dc_init_host); nodes != NULL; nodes = nodes->next)
   {
      ...
      do
      {
         ...
         if(result >= 0)
         {
            new_assignments += result;
            /* Update global booleans for the variable's predicates. */
            evaluatePredicateeggp_dc_init_3(morphism);
            if(!evaluateCondition())
            {
               /* Reset the boolean variables in the predicates of this variable. */
               eggp_dc_init_b3 = true;
               break;
            }
         }
         else break;
   ...
   for(host_index = 0; host_index < eggp_dc_init_host->nodes.size; host_index++)
   {
      ...
      do
      {
         ...
         if(result >= 0)
         {
            new_assignments += result;
            /* Update global booleans for the variable's predicates. */
            evaluatePredicateeggp_dc_init_1(morphism);
            if(!evaluateCondition())
            {
               /* Reset the boolean variables in the predicates of this variable. */
               eggp_dc_init_b1 = true;
               break;
            }
         }
         else break;
...
void applyeggp_dc_init_addEdgeToArity(Morphism *morphism, bool record_changes)
{
   int node_index = lookupNode(morphism, 0);
   int outdegree0 = getOutdegree(eggp_dc_init_host, node_index);
   int host_edge_index;
   int source, target;
   int edge_array_size0 = eggp_dc_init_host->edges.size;
   ...
   host_edge_index = addEdge(eggp_dc_init_host, label, source, target);
   /* If the edge array size has not increased after the edge addition, then
      the edge was added to a hole in the array. */
   if(record_changes)
      pushAddedEdge(host_edge_index, edge_array_size0 == eggp_dc_init_host->edges.size);
   /* Reset the morphism. */
   initialiseMorphism(morphism, eggp_dc_init_host);
}
...
bool fillpoteggp_dc_init_addEdgeToArity(MorphismPot *pot, Morphism *morphism)
{
   if(4 > eggp_dc_init_host->number_of_nodes || 0 > eggp_dc_init_host->number_of_edges) return false;
...
static bool fillpot_n1(MorphismPot *pot, Morphism *morphism)
{
   ...
   for(host_index = 0; host_index < eggp_dc_init_host->nodes.size; host_index++)
   {
      ...
      if(match)
      {
         ...
         host_node->matched = true;
         /* All items matched! */
            putMorphism(pot, morphism, "eggp_dc_init_addEdgeToArity", eggp_dc_init_host);
