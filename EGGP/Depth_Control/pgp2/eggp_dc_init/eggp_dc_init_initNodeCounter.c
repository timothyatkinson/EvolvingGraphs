...
bool matcheggp_dc_init_initNodeCounter(Morphism *morphism)
{
...
void applyeggp_dc_init_initNodeCounter(Morphism *morphism, bool record_changes)
{
   int var_0 = getIntegerValue(morphism, 0);
   int host_node_index;
   int node_array_size1 = eggp_dc_init_host->nodes.size;
   HostLabel label;
   ...
   host_node_index = addNode(eggp_dc_init_host, 1, label);
   /* If the node array size has not increased after the node addition, then
      the node was added to a hole in the array. */
   if(record_changes)
      pushAddedNode(host_node_index, node_array_size1 == eggp_dc_init_host->nodes.size);
   /* Reset the morphism. */
   initialiseMorphism(morphism, eggp_dc_init_host);
}
...
bool fillpoteggp_dc_init_initNodeCounter(MorphismPot *pot, Morphism *morphism)
{
   if(1 > eggp_dc_init_host->number_of_nodes || 0 > eggp_dc_init_host->number_of_edges) return false;
...
static bool fillpot_n0(MorphismPot *pot, Morphism *morphism)
{
   ...
   for(nodes = getRootNodeList(eggp_dc_init_host); nodes != NULL; nodes = nodes->next)
   {
      ...
      if(match)
      {
         ...
         host_node->matched = true;
         /* All items matched! */
            putMorphism(pot, morphism, "eggp_dc_init_initNodeCounter", eggp_dc_init_host);
