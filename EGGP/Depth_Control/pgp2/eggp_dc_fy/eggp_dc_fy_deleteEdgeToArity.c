...
bool eggp_dc_fy_b6 = true;

static bool evaluateCondition(void)
{
   return (eggp_dc_fy_b6);
}

static void evaluatePredicateeggp_dc_fy_6(Morphism *morphism)
{
   ...
   if(var_4 > var_5) eggp_dc_fy_b6 = true;
   else eggp_dc_fy_b6 = false;
}
...
bool matcheggp_dc_fy_deleteEdgeToArity(Morphism *morphism)
{
   if(4 > eggp_dc_fy_host->number_of_nodes || 1 > eggp_dc_fy_host->number_of_edges) return false;
   ...
   for(nodes = getRootNodeList(eggp_dc_fy_host); nodes != NULL; nodes = nodes->next)
   {
      ...
      do
      {
         ...
         if(result >= 0)
         {
            ...
            evaluatePredicateeggp_dc_fy_6(morphism);
...
static bool match_e0(Morphism *morphism)
{
   ...
   Node *host_node = getNode(eggp_dc_fy_host, start_index);

   int counter;
   for(counter = 0; counter < host_node->out_edges.size + 2; counter++)
   {
      Edge *host_edge = getNthOutEdge(eggp_dc_fy_host, host_node, counter);
      ...
      {
         Node *end_node = getNode(eggp_dc_fy_host, host_edge->target);
...
static bool match_n1(Morphism *morphism, Edge *host_edge)
{
   Node *host_node = getTarget(eggp_dc_fy_host, host_edge);
...
void applyeggp_dc_fy_deleteEdgeToArity(Morphism *morphism, bool record_changes)
{
   int var_4 = getIntegerValue(morphism, 4);
   int host_edge_index = lookupEdge(morphism, 0);
   if(record_changes)
   {
      Edge *edge = getEdge(eggp_dc_fy_host, host_edge_index);
      ...
      pushRemovedEdge(edge->label, edge->source, edge->target, edge->index,
                      edge->index < eggp_dc_fy_host->edges.size - 1);
   }
   removeEdge(eggp_dc_fy_host, host_edge_index);

   int host_node_index = lookupNode(morphism, 2);
   HostLabel label_n2 = getNodeLabel(eggp_dc_fy_host, host_node_index);
   HostLabel label;
...
bool fillpoteggp_dc_fy_deleteEdgeToArity(MorphismPot *pot, Morphism *morphism)
{
   if(4 > eggp_dc_fy_host->number_of_nodes || 1 > eggp_dc_fy_host->number_of_edges) return false;
...
static bool fillpot_n1(MorphismPot *pot, Morphism *morphism, Edge *host_edge)
{
   ...
   if(match)
   {
      ...
      host_node->matched = true;
      /* All items matched! */
         putMorphism(pot, morphism, "eggp_dc_fy_deleteEdgeToArity", eggp_dc_fy_host);
