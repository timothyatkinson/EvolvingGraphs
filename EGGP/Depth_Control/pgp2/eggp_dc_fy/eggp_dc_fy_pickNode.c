...
bool eggp_dc_fy_b0 = true;
bool eggp_dc_fy_b1 = true;

static bool evaluateCondition(void)
{
   return (eggp_dc_fy_b0 && eggp_dc_fy_b1);
}

static void evaluatePredicateeggp_dc_fy_0(Morphism *morphism)
{
   ...
   if(!equalHostLists(array0, array1, list_length0, list_length1)) eggp_dc_fy_b0 = true;
   else eggp_dc_fy_b0 = false;
}

static void evaluatePredicateeggp_dc_fy_1(Morphism *morphism)
{
   Assignment assignment_1 = getAssignment(morphism, 1);
   /* If the variable is not yet assigned, return. */
   if(assignment_1.type == 'n') return;
   ...
   if(!equalHostLists(array0, array1, list_length0, list_length1)) eggp_dc_fy_b1 = true;
   else eggp_dc_fy_b1 = false;
}

static bool match_n0(Morphism *morphism);

bool matcheggp_dc_fy_pickNode(Morphism *morphism)
{
   if(1 > eggp_dc_fy_host->number_of_nodes || 0 > eggp_dc_fy_host->number_of_edges) return false;
   ...
   for(host_index = 0; host_index < eggp_dc_fy_host->nodes.size; host_index++)
   {
      ...
      do
      {
         ...
         if(result >= 0)
         {
            ...
            evaluatePredicateeggp_dc_fy_0(morphism);
            evaluatePredicateeggp_dc_fy_1(morphism);
...
void applyeggp_dc_fy_pickNode(Morphism *morphism, bool record_changes)
{
...
bool fillpoteggp_dc_fy_pickNode(MorphismPot *pot, Morphism *morphism)
{
   if(1 > eggp_dc_fy_host->number_of_nodes || 0 > eggp_dc_fy_host->number_of_edges) return false;
...
static bool fillpot_n0(MorphismPot *pot, Morphism *morphism)
{
   ...
   for(host_index = 0; host_index < eggp_dc_fy_host->nodes.size; host_index++)
   {
      ...
      if(match)
      {
         ...
         host_node->matched = true;
         /* All items matched! */
            putMorphism(pot, morphism, "eggp_dc_fy_pickNode", eggp_dc_fy_host);
