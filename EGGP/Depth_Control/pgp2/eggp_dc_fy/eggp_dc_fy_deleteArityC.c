...
bool matcheggp_dc_fy_deleteArityC(Morphism *morphism)
{
...
void applyeggp_dc_fy_deleteArityC(Morphism *morphism, bool record_changes)
{
   int count;
   for(count = 0; count < morphism->edges; count++)
   {
      if(record_changes)
      {
         Edge *edge = getEdge(eggp_dc_fy_host, morphism->edge_map[count].host_index);
      ...
      removeEdge(eggp_dc_fy_host, morphism->edge_map[count].host_index);
   }
   for(count = 0; count < morphism->nodes; count++)
   {
      if(record_changes)
      {
         Node *node = getNode(eggp_dc_fy_host, morphism->node_map[count].host_index);
         /* A hole is created if the node is not at the right-most index of the array. */
         pushRemovedNode(node->root, node->label, node->index,
                         node->index < eggp_dc_fy_host->nodes.size - 1);
      }
      removeNode(eggp_dc_fy_host, morphism->node_map[count].host_index);
   }
...
bool fillpoteggp_dc_fy_deleteArityC(MorphismPot *pot, Morphism *morphism)
{
   if(2 > eggp_dc_fy_host->number_of_nodes || 0 > eggp_dc_fy_host->number_of_edges) return false;
...
static bool fillpot_n1(MorphismPot *pot, Morphism *morphism)
{
   ...
   for(host_index = 0; host_index < eggp_dc_fy_host->nodes.size; host_index++)
   {
      ...
      if(match)
      {
         ...
         host_node->matched = true;
         /* All items matched! */
            putMorphism(pot, morphism, "eggp_dc_fy_deleteArityC", eggp_dc_fy_host);
