...
bool eggp_dc_fy_b5 = true;

static bool evaluateCondition(void)
{
   return (eggp_dc_fy_b5);
}

static void evaluatePredicateeggp_dc_fy_5(Morphism *morphism)
{
   ...
   if(var_4 < var_5) eggp_dc_fy_b5 = true;
   else eggp_dc_fy_b5 = false;
}
...
bool matcheggp_dc_fy_addEdgeToArity(Morphism *morphism)
{
   if(4 > eggp_dc_fy_host->number_of_nodes || 0 > eggp_dc_fy_host->number_of_edges) return false;
   ...
   for(nodes = getRootNodeList(eggp_dc_fy_host); nodes != NULL; nodes = nodes->next)
   {
      ...
      do
      {
         ...
         if(result >= 0)
         {
            ...
            evaluatePredicateeggp_dc_fy_5(morphism);
...
void applyeggp_dc_fy_addEdgeToArity(Morphism *morphism, bool record_changes)
{
   int var_4 = getIntegerValue(morphism, 4);
   ...
   int host_edge_index;
   int source, target;
   int edge_array_size0 = eggp_dc_fy_host->edges.size;
   ...
   host_edge_index = addEdge(eggp_dc_fy_host, label, source, target);
   /* If the edge array size has not increased after the edge addition, then
      the edge was added to a hole in the array. */
   if(record_changes)
      pushAddedEdge(host_edge_index, edge_array_size0 == eggp_dc_fy_host->edges.size);
   /* Reset the morphism. */
   initialiseMorphism(morphism, eggp_dc_fy_host);
}
...
bool fillpoteggp_dc_fy_addEdgeToArity(MorphismPot *pot, Morphism *morphism)
{
   if(4 > eggp_dc_fy_host->number_of_nodes || 0 > eggp_dc_fy_host->number_of_edges) return false;
...
static bool fillpot_n1(MorphismPot *pot, Morphism *morphism)
{
   ...
   for(host_index = 0; host_index < eggp_dc_fy_host->nodes.size; host_index++)
   {
      ...
      if(match)
      {
         ...
         host_node->matched = true;
         /* All items matched! */
            putMorphism(pot, morphism, "eggp_dc_fy_addEdgeToArity", eggp_dc_fy_host);
