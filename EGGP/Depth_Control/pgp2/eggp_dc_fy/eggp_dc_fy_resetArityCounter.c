...
bool matcheggp_dc_fy_resetArityCounter(Morphism *morphism)
{
...
void applyeggp_dc_fy_resetArityCounter(Morphism *morphism, bool record_changes)
{
...
bool fillpoteggp_dc_fy_resetArityCounter(MorphismPot *pot, Morphism *morphism)
{
   if(1 > eggp_dc_fy_host->number_of_nodes || 0 > eggp_dc_fy_host->number_of_edges) return false;
...
static bool fillpot_n0(MorphismPot *pot, Morphism *morphism)
{
   ...
   for(nodes = getRootNodeList(eggp_dc_fy_host); nodes != NULL; nodes = nodes->next)
   {
      ...
      if(match)
      {
         ...
         host_node->matched = true;
         /* All items matched! */
            putMorphism(pot, morphism, "eggp_dc_fy_resetArityCounter", eggp_dc_fy_host);
