...
bool eggp_dc_node_b2 = true;

static bool evaluateCondition(void)
{
   return (eggp_dc_node_b2);
}

static void evaluatePredicateeggp_dc_node_2(Morphism *morphism)
{
   ...
   if(!equalHostLists(array0, array1, list_length0, list_length1)) eggp_dc_node_b2 = true;
   else eggp_dc_node_b2 = false;
}

static bool match_n1(Morphism *morphism);
static bool match_n0(Morphism *morphism);

bool matcheggp_dc_node_mutateNode(Morphism *morphism)
{
   if(2 > eggp_dc_node_host->number_of_nodes || 0 > eggp_dc_node_host->number_of_edges) return false;
...
{
   RootNodes *nodes;
   for(nodes = getRootNodeList(eggp_dc_node_host); nodes != NULL; nodes = nodes->next)
   {
      Node *host_node = getNode(eggp_dc_node_host, nodes->index);
...
void applyeggp_dc_node_mutateNode(Morphism *morphism, bool record_changes)
{
   string var_2 = getStringValue(morphism, 2);
   ...
   else
   {
      ...
      relabelNode(eggp_dc_node_host, host_node_index, label);
   }
   /* Reset the morphism. */
   initialiseMorphism(morphism, eggp_dc_node_host);
}
...
bool fillpoteggp_dc_node_mutateNode(MorphismPot *pot, Morphism *morphism)
{
   if(2 > eggp_dc_node_host->number_of_nodes || 0 > eggp_dc_node_host->number_of_edges) return false;
...
static bool fillpot_n0(MorphismPot *pot, Morphism *morphism)
{
   ...
   for(host_index = 0; host_index < eggp_dc_node_host->nodes.size; host_index++)
   {
      ...
      if(match)
      {
         ...
         host_node->matched = true;
         /* All items matched! */
            putMorphism(pot, morphism, "eggp_dc_node_mutateNode", eggp_dc_node_host);
