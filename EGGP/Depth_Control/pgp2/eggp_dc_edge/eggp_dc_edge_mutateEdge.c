...
#include "eggp_dc_edge.h"

bool eggp_dc_edge_b2 = true;

static bool evaluateCondition(void)
{
   return (eggp_dc_edge_b2);
}

static void evaluatePredicateeggp_dc_edge_2(Morphism *morphism)
{
   ...
   if(!equalHostLists(array0, array1, list_length0, list_length1)) eggp_dc_edge_b2 = true;
   else eggp_dc_edge_b2 = false;
}
...
bool matcheggp_dc_edge_mutateEdge(Morphism *morphism)
{
   if(3 > eggp_dc_edge_host->number_of_nodes || 1 > eggp_dc_edge_host->number_of_edges) return false;
   if(match_n0(morphism)) return true;
   else
   {
      initialiseMorphism(morphism, eggp_dc_edge_host);
      return false;
   }
}
...
{
   int host_index;
   for(host_index = 0; host_index < eggp_dc_edge_host->nodes.size; host_index++)
   {
      Node *host_node = getNode(eggp_dc_edge_host, host_index);
...
static bool match_e0(Morphism *morphism)
{
   ...
   Node *host_node = getNode(eggp_dc_edge_host, start_index);

   int counter;
   for(counter = 0; counter < host_node->out_edges.size + 2; counter++)
   {
      Edge *host_edge = getNthOutEdge(eggp_dc_edge_host, host_node, counter);
      ...
      {
         Node *end_node = getNode(eggp_dc_edge_host, host_edge->target);
...
static bool match_n1(Morphism *morphism, Edge *host_edge)
{
   Node *host_node = getTarget(eggp_dc_edge_host, host_edge);
...
void applyeggp_dc_edge_mutateEdge(Morphism *morphism, bool record_changes)
{
   Assignment var_2 = getAssignment(morphism, 2);
   int host_edge_index = lookupEdge(morphism, 0);
   if(record_changes)
   {
      Edge *edge = getEdge(eggp_dc_edge_host, host_edge_index);
      ...
      pushRemovedEdge(edge->label, edge->source, edge->target, edge->index,
                      edge->index < eggp_dc_edge_host->edges.size - 1);
   }
   removeEdge(eggp_dc_edge_host, host_edge_index);

   int host_node_index = lookupNode(morphism, 0);
   HostLabel label_n0 = getNodeLabel(eggp_dc_edge_host, host_node_index);
   if(record_changes) pushRemarkedNode(host_node_index, label_n0.mark);
   changeNodeMark(eggp_dc_edge_host, host_node_index, 0);

   host_node_index = lookupNode(morphism, 1);
   HostLabel label_n1 = getNodeLabel(eggp_dc_edge_host, host_node_index);
   if(record_changes) pushRemarkedNode(host_node_index, label_n1.mark);
   changeNodeMark(eggp_dc_edge_host, host_node_index, 0);

   int source, target;
   int edge_array_size0 = eggp_dc_edge_host->edges.size;
   ...
   host_edge_index = addEdge(eggp_dc_edge_host, label, source, target);
   /* If the edge array size has not increased after the edge addition, then
      the edge was added to a hole in the array. */
   if(record_changes)
      pushAddedEdge(host_edge_index, edge_array_size0 == eggp_dc_edge_host->edges.size);
   /* Reset the morphism. */
   initialiseMorphism(morphism, eggp_dc_edge_host);
}
...
bool fillpoteggp_dc_edge_mutateEdge(MorphismPot *pot, Morphism *morphism)
{
   if(3 > eggp_dc_edge_host->number_of_nodes || 1 > eggp_dc_edge_host->number_of_edges) return false;
...
static bool fillpot_n2(MorphismPot *pot, Morphism *morphism)
{
   ...
   for(host_index = 0; host_index < eggp_dc_edge_host->nodes.size; host_index++)
   {
      ...
      if(match)
      {
         ...
         host_node->matched = true;
         /* All items matched! */
            putMorphism(pot, morphism, "eggp_dc_edge_mutateEdge", eggp_dc_edge_host);
