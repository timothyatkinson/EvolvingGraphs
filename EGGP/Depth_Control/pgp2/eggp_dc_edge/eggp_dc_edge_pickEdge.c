...
bool matcheggp_dc_edge_pickEdge(Morphism *morphism)
{
   if(2 > eggp_dc_edge_host->number_of_nodes || 1 > eggp_dc_edge_host->number_of_edges) return false;
...
void applyeggp_dc_edge_pickEdge(Morphism *morphism, bool record_changes)
{
   int host_edge_index = lookupEdge(morphism, 0);
   HostLabel label_e0 = getEdgeLabel(eggp_dc_edge_host, host_edge_index);
   if(record_changes) pushRemarkedEdge(host_edge_index, label_e0.mark);
   changeEdgeMark(eggp_dc_edge_host, host_edge_index, 2);
   ...
   changeNodeMark(eggp_dc_edge_host, host_node_index, 2);
...
bool fillpoteggp_dc_edge_pickEdge(MorphismPot *pot, Morphism *morphism)
{
   if(2 > eggp_dc_edge_host->number_of_nodes || 1 > eggp_dc_edge_host->number_of_edges) return false;
...
static bool fillpot_n1(MorphismPot *pot, Morphism *morphism, Edge *host_edge)
{
   ...
   if(match)
   {
      ...
      host_node->matched = true;
      /* All items matched! */
         putMorphism(pot, morphism, "eggp_dc_edge_pickEdge", eggp_dc_edge_host);
