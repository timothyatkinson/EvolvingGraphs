...
bool eggp_dc_edge_b0 = true;
bool eggp_dc_edge_b1 = true;

static bool evaluateCondition(void)
{
   return (eggp_dc_edge_b0 && eggp_dc_edge_b1);
}

static void evaluatePredicateeggp_dc_edge_0(Morphism *morphism)
{
   ...
   if(!equalHostLists(array0, array1, list_length0, list_length1)) eggp_dc_edge_b0 = true;
   else eggp_dc_edge_b0 = false;
}

static void evaluatePredicateeggp_dc_edge_1(Morphism *morphism)
{
   ...
   if(var_7 + var_8 > var_10) eggp_dc_edge_b1 = true;
   else eggp_dc_edge_b1 = false;
}
...
{
   if(4 > eggp_dc_edge_host->number_of_nodes || 1 > eggp_dc_edge_host->number_of_edges) return false;
...
{
   RootNodes *nodes;
   for(nodes = getRootNodeList(eggp_dc_edge_host); nodes != NULL; nodes = nodes->next)
   {
      Node *host_node = getNode(eggp_dc_edge_host, nodes->index);
   ...
   for(host_index = 0; host_index < eggp_dc_edge_host->nodes.size; host_index++)
   {
      ...
      do
      {
         ...
         if(result >= 0)
         {
            new_assignments += result;
            /* Update global booleans for the variable's predicates. */
            evaluatePredicateeggp_dc_edge_0(morphism);
            if(!evaluateCondition())
            {
               /* Reset the boolean variables in the predicates of this variable. */
               eggp_dc_edge_b0 = true;
               break;
            }
         }
         else break;
...
void applyeggp_dc_edge_markDepth(Morphism *morphism, bool record_changes)
{
   ...
   HostLabel label_n2 = getNodeLabel(eggp_dc_edge_host, host_node_index);
   if(record_changes) pushRemarkedNode(host_node_index, label_n2.mark);
   changeNodeMark(eggp_dc_edge_host, host_node_index, 3);
...
static bool fillpot_n2(MorphismPot *pot, Morphism *morphism)
{
   ...
   for(host_index = 0; host_index < eggp_dc_edge_host->nodes.size; host_index++)
   {
      ...
      if(match)
      {
         ...
         host_node->matched = true;
         /* All items matched! */
            putMorphism(pot, morphism, "eggp_dc_edge_markDepth", eggp_dc_edge_host);
