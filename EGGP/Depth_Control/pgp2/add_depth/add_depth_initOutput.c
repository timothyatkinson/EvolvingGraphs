...
bool matchadd_depth_initOutput(Morphism *morphism)
{
   if(1 > add_depth_host->number_of_nodes || 0 > add_depth_host->number_of_edges) return false;
...
void applyadd_depth_initOutput(Morphism *morphism, bool record_changes)
{
   Assignment var_0 = getAssignment(morphism, 0);
   int host_node_index = lookupNode(morphism, 0);
   HostLabel label_n0 = getNodeLabel(add_depth_host, host_node_index);
   HostLabel label;
...
bool fillpotadd_depth_initOutput(MorphismPot *pot, Morphism *morphism)
{
   if(1 > add_depth_host->number_of_nodes || 0 > add_depth_host->number_of_edges) return false;
...
static bool fillpot_n0(MorphismPot *pot, Morphism *morphism)
{
   ...
   for(host_index = 0; host_index < add_depth_host->nodes.size; host_index++)
   {
      ...
      if(match)
      {
         ...
         host_node->matched = true;
         /* All items matched! */
            putMorphism(pot, morphism, "add_depth_initOutput", add_depth_host);
