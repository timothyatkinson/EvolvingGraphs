...
bool matchadd_depth_initInput(Morphism *morphism)
{
...
void applyadd_depth_initInput(Morphism *morphism, bool record_changes)
{
...
bool fillpotadd_depth_initInput(MorphismPot *pot, Morphism *morphism)
{
   if(1 > add_depth_host->number_of_nodes || 0 > add_depth_host->number_of_edges) return false;
...
static bool fillpot_n0(MorphismPot *pot, Morphism *morphism)
{
   ...
   for(host_index = 0; host_index < add_depth_host->nodes.size; host_index++)
   {
      ...
      if(match)
      {
         ...
         host_node->matched = true;
         /* All items matched! */
            putMorphism(pot, morphism, "add_depth_initInput", add_depth_host);
