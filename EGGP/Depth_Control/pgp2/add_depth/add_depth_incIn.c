...
static void evaluatePredicateadd_depth_0(Morphism *morphism)
{
   ...
   if(var_3 < var_5 + 1) add_depth_b0 = true;
   else add_depth_b0 = false;
}
   ...
   for(host_index = 0; host_index < add_depth_host->nodes.size; host_index++)
   {
      ...
      do
      {
         ...
         if(result >= 0)
         {
            ...
            evaluatePredicateadd_depth_0(morphism);
...
static bool fillpot_n1(MorphismPot *pot, Morphism *morphism, Edge *host_edge)
{
   ...
   if(match)
   {
      ...
      host_node->matched = true;
      /* All items matched! */
         putMorphism(pot, morphism, "add_depth_incIn", add_depth_host);
