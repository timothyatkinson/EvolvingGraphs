...
{
   if(2 > n_edge_host->number_of_nodes || 1 > n_edge_host->number_of_edges) return false;
...
void applyn_edge_removeActiveEdge(Morphism *morphism, bool record_changes)
{
   ...
   if(record_changes)
   {
      Edge *edge = getEdge(n_edge_host, host_edge_index);
   ...
   removeEdge(n_edge_host, host_edge_index);
...
static bool fillpot_n0(MorphismPot *pot, Morphism *morphism, Edge *host_edge)
{
   ...
   if(match)
   {
      ...
      host_node->matched = true;
      /* All items matched! */
         putMorphism(pot, morphism, "n_edge_removeActiveEdge", n_edge_host);
