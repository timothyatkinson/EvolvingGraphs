...
static void evaluatePredicaten_edge_3(Morphism *morphism)
{
   ...
   if(!equalHostLists(array0, array1, list_length0, list_length1)) n_edge_b3 = true;
   else n_edge_b3 = false;
}
   ...
   for(host_index = 0; host_index < n_edge_host->nodes.size; host_index++)
   {
      ...
      do
      {
         ...
         if(result >= 0)
         {
            ...
            evaluatePredicaten_edge_3(morphism);
...
void applyn_edge_pickNewTarget(Morphism *morphism, bool record_changes)
{
   ...
   changeNodeMark(n_edge_host, host_node_index, 1);
...
static bool fillpot_n0(MorphismPot *pot, Morphism *morphism)
{
   ...
   for(host_index = 0; host_index < n_edge_host->nodes.size; host_index++)
   {
      ...
      if(match)
      {
         ...
         host_node->matched = true;
         /* All items matched! */
            putMorphism(pot, morphism, "n_edge_pickNewTarget", n_edge_host);
