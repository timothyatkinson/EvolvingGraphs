...
static void evaluatePredicaten_edge_1(Morphism *morphism)
{
...
static bool match_n2(Morphism *morphism)
{
   ...
   for(nodes = getRootNodeList(n_edge_host); nodes != NULL; nodes = nodes->next)
   {
      ...
      if(match)
      {
         ...
         evaluatePredicaten_edge_1(morphism);
...
void applyn_edge_propActive(Morphism *morphism, bool record_changes)
{
   int host_edge_index;
   int source, target;
   int edge_array_size1 = n_edge_host->edges.size;
   ...
   if(record_changes)
      pushAddedEdge(host_edge_index, edge_array_size1 == n_edge_host->edges.size);
   /* Reset the morphism. */
   initialiseMorphism(morphism, n_edge_host);
}
...
static bool fillpot_n1(MorphismPot *pot, Morphism *morphism, Edge *host_edge)
{
   ...
   if(match)
   {
      ...
      {
         /* All items matched! */
         putMorphism(pot, morphism, "n_edge_propActive", n_edge_host);
         /* Reset the boolean variables in the predicates of this node. */
         n_edge_b1 = false;
         removeNodeMap(morphism, 1);
         host_node->matched = false;
