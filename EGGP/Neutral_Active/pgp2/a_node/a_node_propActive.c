...
static void evaluatePredicatea_node_1(Morphism *morphism)
{
   ...
   Node *source = getNode(a_node_host, n2);
...
{
   if(3 > a_node_host->number_of_nodes || 1 > a_node_host->number_of_edges) return false;
...
void applya_node_propActive(Morphism *morphism, bool record_changes)
{
   int host_edge_index;
   int source, target;
   int edge_array_size1 = a_node_host->edges.size;
   ...
   if(record_changes)
      pushAddedEdge(host_edge_index, edge_array_size1 == a_node_host->edges.size);
   /* Reset the morphism. */
   initialiseMorphism(morphism, a_node_host);
}
...
static bool fillpot_n1(MorphismPot *pot, Morphism *morphism, Edge *host_edge)
{
   ...
   if(match)
   {
      ...
      {
         /* All items matched! */
         putMorphism(pot, morphism, "a_node_propActive", a_node_host);
         /* Reset the boolean variables in the predicates of this node. */
         a_node_b1 = false;
         removeNodeMap(morphism, 1);
         host_node->matched = false;
