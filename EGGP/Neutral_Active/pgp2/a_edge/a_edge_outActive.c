...
static void evaluatePredicatea_edge_0(Morphism *morphism)
{
   ...
   Node *source = getNode(a_edge_host, n1);
...
{
   if(2 > a_edge_host->number_of_nodes || 0 > a_edge_host->number_of_edges) return false;
...
static bool fillpot_n0(MorphismPot *pot, Morphism *morphism)
{
   ...
   for(host_index = 0; host_index < a_edge_host->nodes.size; host_index++)
   {
      ...
      if(match)
      {
         ...
         {
            /* All items matched! */
            putMorphism(pot, morphism, "a_edge_outActive", a_edge_host);
            /* Reset the boolean variables in the predicates of this node. */
            a_edge_b0 = false;
            removeNodeMap(morphism, 0);
            host_node->matched = false;
