...
{
   int host_node_index;
   int node_array_size1 = a_edge_host->nodes.size;
   HostLabel label;
   ...
   host_node_index = addNode(a_edge_host, 1, label);
   /* If the node array size has not increased after the node addition, then
      the node was added to a hole in the array. */
   if(record_changes)
      pushAddedNode(host_node_index, node_array_size1 == a_edge_host->nodes.size);
   /* Reset the morphism. */
   initialiseMorphism(morphism, a_edge_host);
}
...
static bool fillpot_n0(MorphismPot *pot, Morphism *morphism)
{
   ...
   for(host_index = 0; host_index < a_edge_host->nodes.size; host_index++)
   {
      ...
      if(match)
      {
         ...
         host_node->matched = true;
         /* All items matched! */
            putMorphism(pot, morphism, "a_edge_initActiveNode", a_edge_host);
