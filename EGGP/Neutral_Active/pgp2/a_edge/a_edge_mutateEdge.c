...
static void evaluatePredicatea_edge_1(Morphism *morphism)
{
   ...
   if(!equalHostLists(array0, array1, list_length0, list_length1)) a_edge_b1 = true;
   else a_edge_b1 = false;
}
...
bool matcha_edge_mutateEdge(Morphism *morphism)
{
...
{
   int host_index;
   for(host_index = 0; host_index < a_edge_host->nodes.size; host_index++)
   {
...
static bool match_e0(Morphism *morphism)
{
   ...
   Node *host_node = getNode(a_edge_host, start_index);

   int counter;
   for(counter = 0; counter < host_node->out_edges.size + 2; counter++)
   {
      Edge *host_edge = getNthOutEdge(a_edge_host, host_node, counter);
      ...
      {
         Node *end_node = getNode(a_edge_host, host_edge->target);
...
static bool match_n1(Morphism *morphism, Edge *host_edge)
{
   Node *host_node = getTarget(a_edge_host, host_edge);
   ...
   for(host_index = 0; host_index < a_edge_host->nodes.size; host_index++)
   {
      ...
      do
      {
         ...
         if(result >= 0)
         {
            ...
            evaluatePredicatea_edge_1(morphism);
...
void applya_edge_mutateEdge(Morphism *morphism, bool record_changes)
{
   Assignment var_2 = getAssignment(morphism, 2);
   int host_edge_index = lookupEdge(morphism, 0);
   if(record_changes)
   {
      Edge *edge = getEdge(a_edge_host, host_edge_index);
   ...
   removeEdge(a_edge_host, host_edge_index);

   int host_node_index = lookupNode(morphism, 0);
   HostLabel label_n0 = getNodeLabel(a_edge_host, host_node_index);
   if(record_changes) pushRemarkedNode(host_node_index, label_n0.mark);
   changeNodeMark(a_edge_host, host_node_index, 0);

   host_node_index = lookupNode(morphism, 1);
   HostLabel label_n1 = getNodeLabel(a_edge_host, host_node_index);
   if(record_changes) pushRemarkedNode(host_node_index, label_n1.mark);
   changeNodeMark(a_edge_host, host_node_index, 0);

   int source, target;
   int edge_array_size0 = a_edge_host->edges.size;
   ...
   host_edge_index = addEdge(a_edge_host, label, source, target);
   /* If the edge array size has not increased after the edge addition, then
      the edge was added to a hole in the array. */
   if(record_changes)
      pushAddedEdge(host_edge_index, edge_array_size0 == a_edge_host->edges.size);
   /* Reset the morphism. */
   initialiseMorphism(morphism, a_edge_host);
}
...
bool fillpota_edge_mutateEdge(MorphismPot *pot, Morphism *morphism)
{
   if(3 > a_edge_host->number_of_nodes || 1 > a_edge_host->number_of_edges) return false;
...
static bool fillpot_n2(MorphismPot *pot, Morphism *morphism)
{
   ...
   for(host_index = 0; host_index < a_edge_host->nodes.size; host_index++)
   {
      ...
      if(match)
      {
         ...
         host_node->matched = true;
         /* All items matched! */
            putMorphism(pot, morphism, "a_edge_mutateEdge", a_edge_host);
