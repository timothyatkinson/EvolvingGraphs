...
{
   int host_node_index;
   int node_array_size1 = n_node_host->nodes.size;
   HostLabel label;
   ...
   host_node_index = addNode(n_node_host, 1, label);
   /* If the node array size has not increased after the node addition, then
      the node was added to a hole in the array. */
   if(record_changes)
      pushAddedNode(host_node_index, node_array_size1 == n_node_host->nodes.size);
   /* Reset the morphism. */
   initialiseMorphism(morphism, n_node_host);
}
...
static bool fillpot_n0(MorphismPot *pot, Morphism *morphism)
{
   ...
   for(host_index = 0; host_index < n_node_host->nodes.size; host_index++)
   {
      ...
      if(match)
      {
         ...
         host_node->matched = true;
         /* All items matched! */
            putMorphism(pot, morphism, "n_node_initActiveNode", n_node_host);
