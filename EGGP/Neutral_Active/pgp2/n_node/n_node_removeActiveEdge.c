...
{
   if(2 > n_node_host->number_of_nodes || 1 > n_node_host->number_of_edges) return false;
...
void applyn_node_removeActiveEdge(Morphism *morphism, bool record_changes)
{
   ...
   if(record_changes)
   {
      Edge *edge = getEdge(n_node_host, host_edge_index);
      ...
      pushRemovedEdge(edge->label, edge->source, edge->target, edge->index,
                      edge->index < n_node_host->edges.size - 1);
   }
   removeEdge(n_node_host, host_edge_index);
...
static bool fillpot_n0(MorphismPot *pot, Morphism *morphism, Edge *host_edge)
{
   ...
   if(match)
   {
      ...
      host_node->matched = true;
      /* All items matched! */
         putMorphism(pot, morphism, "n_node_removeActiveEdge", n_node_host);
