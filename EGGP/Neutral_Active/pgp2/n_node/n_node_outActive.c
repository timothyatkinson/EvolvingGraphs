...
static void evaluatePredicaten_node_0(Morphism *morphism)
{
...
static bool match_n1(Morphism *morphism)
{
   ...
   for(nodes = getRootNodeList(n_node_host); nodes != NULL; nodes = nodes->next)
   {
      ...
      if(match)
      {
         ...
         evaluatePredicaten_node_0(morphism);
...
void applyn_node_outActive(Morphism *morphism, bool record_changes)
{
   int host_edge_index;
   int source, target;
   int edge_array_size0 = n_node_host->edges.size;
   ...
   if(record_changes)
      pushAddedEdge(host_edge_index, edge_array_size0 == n_node_host->edges.size);
   /* Reset the morphism. */
   initialiseMorphism(morphism, n_node_host);
}
...
static bool fillpot_n0(MorphismPot *pot, Morphism *morphism)
{
   ...
   for(host_index = 0; host_index < n_node_host->nodes.size; host_index++)
   {
      ...
      if(match)
      {
         ...
         {
            /* All items matched! */
            putMorphism(pot, morphism, "n_node_outActive", n_node_host);
            /* Reset the boolean variables in the predicates of this node. */
            n_node_b0 = false;
            removeNodeMap(morphism, 0);
            host_node->matched = false;
