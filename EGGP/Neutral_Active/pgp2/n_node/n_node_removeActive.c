...
{
   if(1 > n_node_host->number_of_nodes || 0 > n_node_host->number_of_edges) return false;
...
void applyn_node_removeActive(Morphism *morphism, bool record_changes)
{
   ...
   for(count = 0; count < morphism->edges; count++)
   {
      if(record_changes)
      {
         Edge *edge = getEdge(n_node_host, morphism->edge_map[count].host_index);
      ...
      removeEdge(n_node_host, morphism->edge_map[count].host_index);
   }
   for(count = 0; count < morphism->nodes; count++)
   {
      if(record_changes)
      {
         Node *node = getNode(n_node_host, morphism->node_map[count].host_index);
         /* A hole is created if the node is not at the right-most index of the array. */
         pushRemovedNode(node->root, node->label, node->index,
                         node->index < n_node_host->nodes.size - 1);
      }
      removeNode(n_node_host, morphism->node_map[count].host_index);
   }
...
static bool fillpot_n0(MorphismPot *pot, Morphism *morphism)
{
   ...
   for(nodes = getRootNodeList(n_node_host); nodes != NULL; nodes = nodes->next)
   {
      ...
      if(match)
      {
         ...
         host_node->matched = true;
         /* All items matched! */
            putMorphism(pot, morphism, "n_node_removeActive", n_node_host);
