...
static void evaluatePredicaten_node_5(Morphism *morphism)
{
   ...
   if(!equalHostLists(array0, array1, list_length0, list_length1)) n_node_b5 = true;
   else n_node_b5 = false;
}
   ...
   for(nodes = getRootNodeList(n_node_host); nodes != NULL; nodes = nodes->next)
   {
      ...
      do
      {
         ...
         if(result >= 0)
         {
            ...
            evaluatePredicaten_node_5(morphism);
...
void applyn_node_mutateNode(Morphism *morphism, bool record_changes)
{
   ...
   else
   {
      ...
      relabelNode(n_node_host, host_node_index, label);
   }
   /* Reset the morphism. */
   initialiseMorphism(morphism, n_node_host);
}
...
static bool fillpot_n0(MorphismPot *pot, Morphism *morphism)
{
   ...
   for(host_index = 0; host_index < n_node_host->nodes.size; host_index++)
   {
      ...
      if(match)
      {
         ...
         host_node->matched = true;
         /* All items matched! */
            putMorphism(pot, morphism, "n_node_mutateNode", n_node_host);
