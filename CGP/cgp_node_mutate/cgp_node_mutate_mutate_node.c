...
#include "cgp_node_mutate.h"

bool cgp_node_mutate_b0 = true;

static bool evaluateCondition(void)
{
   return (cgp_node_mutate_b0);
}

static void evaluatePredicatecgp_node_mutate_0(Morphism *morphism)
{
   ...
   if(!equalHostLists(array0, array1, list_length0, list_length1)) cgp_node_mutate_b0 = true;
   else cgp_node_mutate_b0 = false;
}
...
{
   if(2 > cgp_node_mutate_host->number_of_nodes || 0 > cgp_node_mutate_host->number_of_edges) return false;
   if(match_n0(morphism)) return true;
   else
   {
      initialiseMorphism(morphism, cgp_node_mutate_host);
      return false;
   }
}
...
{
   RootNodes *nodes;
   for(nodes = getRootNodeList(cgp_node_mutate_host); nodes != NULL; nodes = nodes->next)
   {
      Node *host_node = getNode(cgp_node_mutate_host, nodes->index);
      ...
      do
      {
         ...
         if(result >= 0)
         {
            ...
            evaluatePredicatecgp_node_mutate_0(morphism);
            if(!evaluateCondition())
            {
               /* Reset the boolean variables in the predicates of this variable. */
               cgp_node_mutate_b0 = true;
               break;
            }
         }
         else break;
...
{
   int host_index;
   for(host_index = 0; host_index < cgp_node_mutate_host->nodes.size; host_index++)
   {
      Node *host_node = getNode(cgp_node_mutate_host, host_index);
...
void applycgp_node_mutate_mutate_node(Morphism *morphism, bool record_changes)
{
   ...
   HostLabel label_n1 = getNodeLabel(cgp_node_mutate_host, host_node_index);
   HostLabel label;
   int list_var_length0 = 0;
   ...
   else
   {
      ...
      relabelNode(cgp_node_mutate_host, host_node_index, label);
   }
   /* Reset the morphism. */
   initialiseMorphism(morphism, cgp_node_mutate_host);
}
...
static bool fillpot_n1(MorphismPot *pot, Morphism *morphism)
{
   ...
   for(host_index = 0; host_index < cgp_node_mutate_host->nodes.size; host_index++)
   {
      ...
      if(match)
      {
         ...
         host_node->matched = true;
         /* All items matched! */
            putMorphism(pot, morphism, "cgp_node_mutate_mutate_node", cgp_node_mutate_host);
