...
bool cgp_init_b1 = true;
bool cgp_init_b2 = true;
bool cgp_init_b3 = true;

static bool evaluateCondition(void)
{
   return ((cgp_init_b1 && cgp_init_b2) && cgp_init_b3);
}

static void evaluatePredicatecgp_init_1(Morphism *morphism)
{
   ...
   if(getOutdegree(cgp_init_host, n0) < var_2) cgp_init_b1 = true;
   else cgp_init_b1 = false;
}

static void evaluatePredicatecgp_init_2(Morphism *morphism)
{
   ...
   if(var_1 < var_0) cgp_init_b2 = true;
   else cgp_init_b2 = false;
}

static void evaluatePredicatecgp_init_3(Morphism *morphism)
{
   ...
   array1[index1++].str = var_7;

   if(equalHostLists(array0, array1, list_length0, list_length1)) cgp_init_b3 = true;
   else cgp_init_b3 = false;
}
   ...
   for(nodes = getRootNodeList(cgp_init_host); nodes != NULL; nodes = nodes->next)
   {
      ...
      do
      {
         ...
         if(result >= 0)
         {
            ...
            evaluatePredicatecgp_init_3(morphism);
         ...
         if(result >= 0)
         {
            new_assignments += result;
            /* Update global booleans for the variable's predicates. */
            evaluatePredicatecgp_init_2(morphism);
            if(!evaluateCondition())
            {
               /* Reset the boolean variables in the predicates of this variable. */
               cgp_init_b2 = true;
      ...
      if(match)
      {
         ...
         evaluatePredicatecgp_init_1(morphism);
...
static bool match_n1(Morphism *morphism)
{
   ...
   for(host_index = 0; host_index < cgp_init_host->nodes.size; host_index++)
   {
      ...
      do
      {
         ...
         item = item->prev;
         /* Matching list variable 4. */
         if(item == start->prev) result = addListAssignment(morphism, 4, NULL);
         else if(item == start)
         {
            if(item->atom.type == 'i') result = addIntegerAssignment(morphism, 4, item->atom.num);
            else result = addStringAssignment(morphism, 4, item->atom.str);
         }
         else
         {
            /* Assign to variable 4 the unmatched sublist of the host list. */
            HostAtom sublist[label.length - 2];
            int list_index = 0;
            HostListItem *iterator = start;
            while(iterator != item->next)
            ...
            HostList *list = makeHostList(sublist, label.length - 2, false);
            result = addListAssignment(morphism, 4, list);
            freeHostList(list);
         }
         if(result >= 0)
         {
...
void applycgp_init_add_edge_to_arity(Morphism *morphism, bool record_changes)
{
   ...
   int outdegree0 = getOutdegree(cgp_init_host, node_index);
   int host_edge_index;
   int source, target;
   int edge_array_size0 = cgp_init_host->edges.size;
   ...
   host_edge_index = addEdge(cgp_init_host, label, source, target);
   /* If the edge array size has not increased after the edge addition, then
      the edge was added to a hole in the array. */
   if(record_changes)
      pushAddedEdge(host_edge_index, edge_array_size0 == cgp_init_host->edges.size);
   /* Reset the morphism. */
   initialiseMorphism(morphism, cgp_init_host);
}
...
static bool fillpot_n1(MorphismPot *pot, Morphism *morphism)
{
   ...
   for(host_index = 0; host_index < cgp_init_host->nodes.size; host_index++)
   {
      ...
      if(match)
      {
         ...
         host_node->matched = true;
         /* All items matched! */
            putMorphism(pot, morphism, "cgp_init_add_edge_to_arity", cgp_init_host);
