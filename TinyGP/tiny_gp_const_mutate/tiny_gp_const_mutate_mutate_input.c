...
#include "tiny_gp_const_mutate.h"
...
bool matchtiny_gp_const_mutate_mutate_input(Morphism *morphism)
{
   if(3 > tiny_gp_const_mutate_host->number_of_nodes || 1 > tiny_gp_const_mutate_host->number_of_edges) return false;
   if(match_n0(morphism)) return true;
   else
   {
      initialiseMorphism(morphism, tiny_gp_const_mutate_host);
      return false;
   }
}
...
{
   int host_index;
   for(host_index = 0; host_index < tiny_gp_const_mutate_host->nodes.size; host_index++)
   {
      Node *host_node = getNode(tiny_gp_const_mutate_host, host_index);
...
static bool match_e0(Morphism *morphism)
{
   ...
   Node *host_node = getNode(tiny_gp_const_mutate_host, start_index);

   int counter;
   for(counter = 0; counter < host_node->out_edges.size + 2; counter++)
   {
      Edge *host_edge = getNthOutEdge(tiny_gp_const_mutate_host, host_node, counter);
      ...
      {
         Node *end_node = getNode(tiny_gp_const_mutate_host, host_edge->target);
...
static bool match_n1(Morphism *morphism, Edge *host_edge)
{
   Node *host_node = getTarget(tiny_gp_const_mutate_host, host_edge);
...
void applytiny_gp_const_mutate_mutate_input(Morphism *morphism, bool record_changes)
{
   Assignment var_3 = getAssignment(morphism, 3);
   int host_edge_index = lookupEdge(morphism, 0);
   if(record_changes)
   {
      Edge *edge = getEdge(tiny_gp_const_mutate_host, host_edge_index);
      ...
      pushRemovedEdge(edge->label, edge->source, edge->target, edge->index,
                      edge->index < tiny_gp_const_mutate_host->edges.size - 1);
   }
   removeEdge(tiny_gp_const_mutate_host, host_edge_index);

   int source, target;
   int edge_array_size0 = tiny_gp_const_mutate_host->edges.size;
   ...
   host_edge_index = addEdge(tiny_gp_const_mutate_host, label, source, target);
   /* If the edge array size has not increased after the edge addition, then
      the edge was added to a hole in the array. */
   if(record_changes)
      pushAddedEdge(host_edge_index, edge_array_size0 == tiny_gp_const_mutate_host->edges.size);
   /* Reset the morphism. */
   initialiseMorphism(morphism, tiny_gp_const_mutate_host);
}
...
bool fillpottiny_gp_const_mutate_mutate_input(MorphismPot *pot, Morphism *morphism)
{
   if(3 > tiny_gp_const_mutate_host->number_of_nodes || 1 > tiny_gp_const_mutate_host->number_of_edges) return false;
...
static bool fillpot_n2(MorphismPot *pot, Morphism *morphism)
{
   ...
   for(host_index = 0; host_index < tiny_gp_const_mutate_host->nodes.size; host_index++)
   {
      ...
      if(match)
      {
         ...
         host_node->matched = true;
         /* All items matched! */
            putMorphism(pot, morphism, "tiny_gp_const_mutate_mutate_input", tiny_gp_const_mutate_host);
